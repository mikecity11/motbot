...
export async function POST(request:Request){
 ...
 try{
  ...
  const completed=response(reply,'ai',200,{generationId:id,generationUrl:`/chat/${id}`,model:result.model,tradeCandidate:result.output.intent==='trade_preview'?executableTradeCandidate(result.output.trade,input.settings):null});completed.cookies.set('mot.ai.session',token,{httpOnly:true,secure:new URL(request.url).protocol==='https:',sameSite:'strict',path:'/',maxAge:2592000});return completed;
